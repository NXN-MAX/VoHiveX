...
镜像由 GitHub Actions 按 `linux/amd64`、`linux/arm64`、`linux/arm/v7` 分别构建，运行启动测试后发布到 `ghcr.io/nxn-max/vohivex`。拉取 `2.1.1` 或 `latest` 时会自动选择架构。
...
docker build -f Dockerfile.vohivex -t vohivex:2.1.1 .
```
