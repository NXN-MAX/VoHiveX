  ...
  <div class="app-shell" :class="{ 'is-collapsed': ui.collapsed }">
    ...
    <aside class="sidebar" :class="{ 'mobile-open': ui.mobileOpen }">
      <div class="sidebar-brand">
        ...
        <button class="icon-button desktop-collapse" :aria-label="ui.collapsed ? '展开导航' : '收起导航'" @click="ui.toggleCollapsed">
          <RiIcon :name="ui.collapsed ? 'menu-unfold-line' : 'menu-fold-line'" />
        </button>
        <button class="icon-button mobile-close" aria-label="关闭导航" @click="closeMobile">
          <RiIcon name="close-line" />
        </button>
      </div>

      <nav class="sidebar-nav" aria-label="主导航">
        <Tooltip v-for="item in items" :key="item.to" :title="ui.collapsed ? item.label : ''" placement="right" overlay-class-name="nav-tooltip">
    ...
    <section class="workspace">
      <header class="mobile-header">
        <button class="icon-button" aria-label="打开导航" @click="ui.mobileOpen = true">
          <RiIcon name="menu-unfold-line" />
        </button>
        <span class="mobile-brand">VoHive<span>X</span></span>
        <div class="mobile-topbar-tools">
          <button class="appearance-switch" :aria-label="ui.dark ? '切换为浅色模式' : '切换为深色模式'" @click="ui.toggleTheme">
            <span class="appearance-track">
              <span class="appearance-thumb">
                <RiIcon :name="ui.dark ? 'moon-line' : 'sun-line'" :size="13" />
              </span>
            </span>
          </button>
          <span
