...
- Docker Hub：`maxnxxn/vohivex:2.0.0`
- GHCR：`ghcr.io/nxn-max/vohivex:2.0.0`
- 通用标签：`2.0.0`、`v2.0.0`、`latest`，自动匹配 AMD64、ARM64、ARMv7。
- 独立标签：`2.0.0-amd64`、`2.0.0-arm64`、`2.0.0-armv7`。

```sh
docker pull maxnxxn/vohivex:2.0.0
```

从 [GitHub 仓库](https://github.com/NXN-MAX/VoHiveX) 获取 `docker-compose.yml` 与 `.env.example`。将 Compose 的 `image` 改为 `maxnxxn/vohivex:2.0.0`，复制 `.env.example` 为 `.env`，填写主机内核版本 `DRIVER_KERNEL` 和监听地址 `VOHIVE_BIND_IP`，然后执行 `docker compose pull && docker compose up -d`。
