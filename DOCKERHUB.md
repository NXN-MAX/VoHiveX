...
- Docker Hub：`maxnxxn/vohivex:2.1.2`
- GHCR：`ghcr.io/nxn-max/vohivex:2.1.2`
- 通用标签：`2.1.2`、`v2.1.2`、`latest`，自动匹配 AMD64、ARM64、ARMv7。
- 独立标签：`2.1.2-amd64`、`2.1.2-arm64`、`2.1.2-aarch64`、`2.1.2-armv7`。

```sh
docker pull maxnxxn/vohivex:2.1.2
```

从 [GitHub 仓库](https://github.com/NXN-MAX/VoHiveX) 获取 `docker-compose.yml` 与 `.env.example`。将 Compose 的 `image` 改为 `maxnxxn/vohivex:2.1.2`，复制 `.env.example` 为 `.env`，填写监听地址 `VOHIVE_BIND_IP`（内核版本自动识别，无需填写），然后执行 `docker compose pull && docker compose up -d`。
